...
// Real SQL Server integration tests for IdentityOutboxStore (OUTBOX-003..006, DATA-006/008). Each test
...
public class IdentityOutboxStoreTests : IClassFixture<MsSqlContainerFixture>
{
    private readonly MsSqlContainerFixture _fixture;

    public IdentityOutboxStoreTests(MsSqlContainerFixture fixture)
    {
        _fixture = fixture;
    }

    private async Task<IdentityDbContext> CreateContextAsync(string databaseName)
    {
    ...
    {
        Id = Guid.NewGuid(),
        ContractType = "Identity.AccountCreated",
        ContractVersion = 1,
        Payload = "{}",
    ...
    public async Task ClaimPendingBatchAsync_ClaimsOldestPendingMessagesFirst_UpToBatchSize()
    {
        ...
        var store = new IdentityOutboxStore(context);
    ...
    public async Task ClaimPendingBatchAsync_ConcurrentClaims_NeverClaimTheSameMessageTwice()
    {
        ...
        var storeA = new IdentityOutboxStore(contextA);
        var storeB = new IdentityOutboxStore(contextB);
    ...
    public async Task MarkDispatchedAsync_SetsDispatchedStatusAndClearsLease_AndTheMessageIsNoLongerClaimable()
    {
        ...
        var storeForReclaim = new IdentityOutboxStore(verifyContext);
