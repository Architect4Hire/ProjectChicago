...
// Timer-triggered outbox relay entry point for CRM (OUTBOX-003..006, ASYNC-001..008; functions.md
...
// IOutboxRelay (ProjectChicago.Shared) owns batch claim/publish/settle against CRM's own outbox
...
// CrmOutboxStore (ProjectChicago.Crm.Core.Persistence) against CrmDbContext.
public sealed class RelayOutboxFunction
{
    ...
    public async Task RunAsync(
        [TimerTrigger("%Crm:OutboxRelay:Schedule%")] TimerInfo timer,
        CancellationToken cancellationToken)
    {
        ...
        _logger.LogInformation(
            "CRM outbox relay claimed {ClaimedCount}, dispatched {DispatchedCount}, failed {FailedCount}.",
